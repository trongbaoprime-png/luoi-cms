...
export default function NewArticlePage() {
  ...
  const handleTitleChange = (val: string) => {
    ...
    // Auto-generate optimized Meta Title under 60 characters
    let trimmedTitle = val.trim();
    if (trimmedTitle.length > 55) {
      trimmedTitle = trimmedTitle.slice(0, 52) + "...";
    } else if (trimmedTitle && !trimmedTitle.includes("Lười Dọn Nhà") && (trimmedTitle + " - Lười Dọn Nhà").length <= 60) {
      trimmedTitle = `${trimmedTitle} - Lười Dọn Nhà`;
    }
    setSeoTitle(trimmedTitle);
  };
  ...
  const handleAutoSuggestKeyword = () => {
    ...
    const cleanKw = title
      .replace(/^[0-9\s\.\,\-]+/, "")
      .replace(/(hướng dẫn|mẹo|bí quyết|cách|10|top|mới nhất|chi tiết|a-z)/gi, "")
      .replace(/\s+/g, " ")
      .trim();
    const suggested = cleanKw || title;
  ...
  const handleAutoGenerateSeo = () => {
    ...
    let kw = focusKeyword ? focusKeyword.trim() : "";
    if (!kw) {
      const cleanKw = title
        .replace(/^[0-9\s\.\,\-]+/, "")
        .replace(/(hướng dẫn|mẹo|bí quyết|cách|10|top|mới nhất|chi tiết|a-z)/gi, "")
    ...
    let autoTitle = title;
    if (kw && !autoTitle.toLowerCase().includes(kw.toLowerCase())) {
      autoTitle = `${kw} - ${autoTitle}`;
    }
    ...
    if (!autoDesc.includes("✓")) {
      autoDesc = `${autoDesc} ✓ Xem ngay tại Lười Dọn Nhà!`;
    }
  ...
  const handleGenerateAI = async () => {
    ...
    setMsg("Trợ lý AI đang tạo bài viết chuẩn SEO Rank Math...");

    try {
      const res = await fetch("/api/content/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topic: title, focusKeyword, location: geoLocation }),
      });
      ...
      if (data.success && data.data) {
        ...
        setSeoDesc(data.data.seoDescription || "");
        if (data.data.focusKeyword) setFocusKeyword(data.data.focusKeyword);
        setMsg("✓ Đã khởi tạo bài viết SEO AI thành công!");
        setEditorTab("ckeditor");
      }
    } finally {
  ...
  return (
    <div className="w-full max-w-[1536px] mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-stone-200 shadow-sm sticky top-2 z-30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-stone-900 font-serif">
              Soạn Bài Viết Mới (Trình Soạn Thảo Nâng Cao)
            </h1>
            <p className="text-xs text-stone-500 font-mono">
              Đầy đủ tính năng Canh Lề, Màu sắc, Bảng, Thư viện Media • Rank Math SEO Score
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl border border-stone-300 bg-white text-stone-700 text-xs font-bold hover:bg-stone-50"
          >
            Lưu Nháp
          </button>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-5 py-2 rounded-xl bg-[#0d9488] text-white text-xs font-bold hover:bg-[#0f766e] shadow-sm"
          >
            <Save size={14} />
      ...
      {msg && (
        <div className="p-3 bg-emerald-50 text-emerald-800 rounded-xl text-xs font-semibold flex items-center justify-between gap-2">
          ...
          {slug && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-3 py-1 bg-[#0d9488] text-white font-bold text-xs rounded-lg hover:bg-[#0f766e] transition-colors flex items-center gap-1"
            >
              <ExternalLink size={12} /> Xem bài viết vừa đăng
            </a>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="VD: 10 Mẹo Vệ Sinh Căn Hộ Sạch Bóng Dành Cho Người Lười..."
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Mô tả tóm tắt gây ấn tượng với độc giả..."
        ...
        <div className="md:col-span-4 space-y-6">
          {/* Featured Image Thumbnail Picker Box */}
            ...
            {coverImage ? (
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-teal-50 text-[#0d9488] font-bold text-xs rounded-xl border border-teal-200 hover:bg-teal-100 flex items-center justify-center gap-1.5"
                >
          ...
          {/* Rank Math SEO Real-Time Score Meter Suite */}
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-[#0d9488] hover:underline flex items-center gap-1"
                  >
                    <Sparkles size={11} /> Gợi ý từ Tiêu đề
                  </button>
                </div>
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="VD: meo di cho tiet kiem"
                    value={focusKeyword}
                    onChange={(e) => setFocusKeyword(e.target.value)}
                    className="w-full pl-8 pr-3 py-2 border border-stone-300 rounded-xl font-bold text-stone-900 focus:outline-none focus:ring-2 focus:ring-[#0d9488]"
                  />
                </div>
              </div>
